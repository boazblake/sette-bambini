  ...
  var addToCart = function addToCart(mdl) {
    return function (product) {
      return function (state) {
        mdl.cart[product][state.gender] += parseInt(state.quantity); //if (mdl.state.isAuth()) => save to db
...
var Product = function Product(_ref4) {
  ...
  return {
    view: function view(_ref5) {
      ...
      return amount ? m(".frow row-around", [m("h3", "".concat(title, ": ").concat(amount, " for ").concat(mdl.state.currency(), " ").concat(price)), m(".frow", genders.map(function (gender) {
        return m(Gender, {
          mdl: mdl,
          gender: gender
        });
...
var getTotal = function getTotal(mdl, currency, products) {
  var getTotalPrice = getQuantity(products.map(function (p) {
    return getPrice(mdl, p[0], p[1]);
  }));
  return getTotalPrice ? m("h1.bold", "Total of ".concat(getQuantity(products), " for ").concat(currency, ": ").concat(getTotalPrice)) : m("h1.m-30", "Your Cart is Empty");
};
  ...
  return {
    ...
    view: function view(_ref8) {
      ...
      return m(".modalOverlay-right.animated", {
        ...
        id: "cart-modal"
      }, [m("h1.title", "Shopping Cart"), products(mdl.cart).map(function (p) {
        return m(Product, {
          mdl: mdl,
          p: p
        });
...
if (localStorage.getItem("sb-user")) {
  _Models["default"].user = JSON.parse(localStorage.getItem("sb-user"));

  _Models["default"].state.isAuth(true); //fetch cart from db.
